...
let rec eval4 ?(mode=0) e env =           (* env を引数に追加 *)
  ...
  | If(e1,e2,e3) ->
      begin
        match (eval4 e1 env ~mode) with          (* env を追加 *)
          | BoolVal(true)  -> eval4 e2 env   (* env を追加 *)
          | BoolVal(false) -> eval4 e3 env   (* env を追加 *)
          | _ -> failwith "wrong value"
      end
  | Let(x,e1,e2) -> 
      let env1 = ext env x (eval4 e1 env ~mode)
      in eval4 e2 env1 ~mode
  | Noteq(e1, e2) ->
      begin
	match (eval4 e1 env ~mode, eval4 e2 env ~mode) with
	  | (IntVal(n1),IntVal(n2)) -> BoolVal(n1!=n2)
